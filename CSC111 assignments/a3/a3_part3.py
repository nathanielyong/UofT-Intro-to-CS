...
def create_book_graph(review_graph: WeightedGraph,
    ...
    """
    g = WeightedGraph()
    for v in review_graph.get_all_vertices('book'):
        g.add_vertex(v, 'book')

    for u in g.get_all_vertices('book'):
        for v in g.get_all_vertices('book'):
            if u != v:
                weight = review_graph.get_similarity_score(u, v, score_type)
                if weight > threshold:
...
def cross_cluster_weight(book_graph: WeightedGraph, cluster1: set, cluster2: set) -> float:
    ...
    for v1 in cluster1:
        for v2 in cluster2:
            n += book_graph.get_weight(v1, v2)
    d = len(cluster1) * len(cluster2)
    return n / d
