...
class _WeightedVertex:
    ...
    def similarity_score_unweighted(self, other: _WeightedVertex) -> float:
        ...
        if self.degree() == 0 or other.degree() == 0:
            ...
            return len(self_neighbours.intersection(other_neighbours)) / len(self_neighbours.union(other_neighbours))
    ...
    def similarity_score_strict(self, other: _WeightedVertex) -> float:
        ...
        if self.degree() == 0 or other.degree() == 0:
            ...
            self_n = set(self.neighbours.keys())
            other_n = set(other.neighbours.keys())
            intersection = {v for v in self_n.intersection(other_n) if self.neighbours[v] == other.neighbours[v]}
            return len(intersection) / len(self_n.union(other_n))
    ...
    def recommend_books(self, book: str, limit: int,
        ...
        *descending order* of similarity score. Ties are broken in descending order
        ...
        for item in self.get_all_vertices('book'):
            if book != item:
                score = self.get_similarity_score(book, item, score_type)
                if score != 0:
    ...
    with open(reviews_file) as csv_file:
        ...
        for user_id, book_id, score in reader:
            if book_id in books:
                ...
                g.add_edge(books[book_id], user_id, int(score))
