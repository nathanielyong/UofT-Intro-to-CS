...
class GameTree:
    ...
    Instance Attributes:
      ...
      - white_win_probability: the probability that White will win from
    ...
    def _str_indented(self, depth: int) -> str:
        ...
        move_desc = f'{self.move} -> {turn_desc} -> {self.white_win_probability}\n'
        s = '  ' * depth + move_desc
        if self._subtrees == []:
            return s
        else:
        ...
        Implementation Notes:
            ...
            - Your implementation must have a worst-case running time of Theta(m + n) time,
              ...
              i) Use a recursive helper method that takes an extra "current index" argument to
                 keep track of the neixt move in the list.
              ii) First reverse the lst, and then use a recursive helper method that calls
                 `list.pop` on the list of moves. Just make sure the original list isn't changed
                 when the function ends!
        """
        self.insert_move_sequence_helper(list(reversed(moves)), False, white_win_probability)

    def insert_move_sequence_helper(self, moves: list[str], is_white_move: bool,
                                    white_win_probability: float = 0.0) -> None:
        """Recursively inserts the given move sequence into this tree.
        """
        if moves == []:
            return
        move = moves.pop()
        ...
        if subtree is not None:
            subtree.insert_move_sequence_helper(moves, not is_white_move, white_win_probability)
            self._update_white_win_probability()
        elif moves == []:
            self.add_subtree(GameTree(move, is_white_move, white_win_probability))
        else:
            game_tree = GameTree(move, is_white_move)
            game_tree.insert_move_sequence_helper(moves, not is_white_move, white_win_probability)
