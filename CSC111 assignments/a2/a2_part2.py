...
def generate_complete_game_tree(root_move: str, game_state: a2_minichess.MinichessGame,
    ...
    if d == 0 or game_state.get_valid_moves() == []:
        if game_state.get_winner() == 'White':
            game_tree = a2_game_tree.GameTree(root_move, game_state.is_white_move(), 1.0)
        else:
            game_tree = a2_game_tree.GameTree(root_move, game_state.is_white_move(), 0.0)
        return game_tree
    else:
...
def part2_runner(d: int, n: int, white_greedy: bool) -> None:
    ...
    """
    game_tree = generate_complete_game_tree(a2_game_tree.GAME_START_MOVE,
                                            a2_minichess.MinichessGame(), d)
